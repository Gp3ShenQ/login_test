<template>
  <loginComponentsLoginBox v-model:userAccount="userAccount" v-model:userPassword="userPassword" @login="userAction('login')" @register="userAction('register')" @fix="userAction('fix')" />
...
import userApiStore from '@/utils/api/userApiStore'
...
const { func_CheckUserPersonalPost } = userApiStore()
...
const login = async () => {
  ...
  if (_result.statusCode == 200) {
    Swal.fire({
      ...
      user.value = await func_CheckUserPersonalPost(userAccount.value)
      navigateTo('/chatView')
      userPassword.value = ''
      userAccount.value = ''
    })
